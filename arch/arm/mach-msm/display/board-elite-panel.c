...
static struct dsi_cmd_desc sony_panel_video_mode_cmds_id18103_ver008[] = {
	...
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x07, 0x27}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x08, 0x25}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x09, 0x21}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x0A, 0x1C}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x0B, 0x14}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x0C, 0x0C}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x0D, 0x06}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x0E, 0x02}},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, 2, (char[]){0x0F, 0x00}},
...
static char himax_vsync_start[2] = {0x00, 0x00}; 
static char himax_hsync_start[2] = {0x00, 0x00}; 
...
static void elite_self_refresh_switch(int on)
{
	...
	if (on) {
		...
		cmdreq.cmds = video_to_cmd;
		cmdreq.cmds_cnt = ARRAY_SIZE(video_to_cmd);
		cmdreq.flags = CMD_REQ_COMMIT;
		cmdreq.rlen = 0;
		cmdreq.cb = NULL;
		mipi_dsi_cmdlist_put(&cmdreq);
		...
		if (panel_type != PANEL_ID_ELITE_SHARP_HX) {
			...
			cmdreq.cmds = cmd_to_video;
			cmdreq.cmds_cnt = ARRAY_SIZE(cmd_to_video);
			cmdreq.flags = CMD_REQ_COMMIT;
			cmdreq.rlen = 0;
			cmdreq.cb = NULL;
			mipi_dsi_cmdlist_put(&cmdreq);
			...
			udelay(300);
			cmdreq.cmds = vsync_hsync_cmds;
			cmdreq.cmds_cnt = ARRAY_SIZE(vsync_hsync_cmds);
			cmdreq.flags = CMD_REQ_COMMIT;
			cmdreq.rlen = 0;
			cmdreq.cb = NULL;
			mipi_dsi_cmdlist_put(&cmdreq);
...
static void elite_mipi_dsi_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		if(!mipi_lcd_on) {
			if (panel_type == PANEL_ID_ELITE_SONY_NT
				...
				cmdreq.cmds = elite_video_on_cmds;
				cmdreq.cmds_cnt = elite_video_on_cmds_count;
				cmdreq.flags = CMD_REQ_COMMIT;
				cmdreq.rlen = 0;
				cmdreq.cb = NULL;
				mipi_dsi_cmdlist_put(&cmdreq);
...
static void elite_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	elite_mipi_dsi_set_backlight(mfd);
...
static int mipi_video_sony_hd720p_init(void)
{
	...
	PR_DISP_INFO("%s: enter mipi_video_sony init.\n", __func__);
		...
		elite_panel_data.self_refresh_switch = NULL; 
#endif
...
extern char mipi_dsi_splash_is_enabled(void);
